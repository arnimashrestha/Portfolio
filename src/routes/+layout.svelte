<script>
  ...
  >
    <div class= {"fixed bottom-0 w-full duration-200 flex p-10 z-[10]"+ (
        y>0? 'opacity-full pointer-events-auto': 'pointer-events-non opacity-0'
    )}>
        <button on:click={goTop} class= "ml-auto rounded-full bd-slate-900 text-violet-400 px-3 sm:px-4 hover:bg-slate-800 cursor-pointer">
